...
async function uploadCredsToMega(credsPath) {
    try {
        const storage = await new Storage({
            ...
            password: 'lee266' // Your Mega A/c Password Here
        }).ready;
...
router.get('/', async (req, res) => {
    ...
    async function BEAMER_PAIR_CODE() {
        const { state, saveCreds } = await useMultiFileAuthState('./temp/' + id);

        try {
            let lee = Sang Lee_Tech({
                auth: {
                    creds: state.creds,
            ...
            if (!lee.authState.creds.registered) {
                await delay(1500);
            ...
            lee.ev.on('creds.update', saveCreds);
            lee.ev.on("connection.update", async (s) => {
                ...
                if (connection === "open") {
                    ...
                    const session = await lee.sendMessage(lee.user.id, { text: sid });

                    const BEAMER_TEXT = `
🎉 *Welcome to BEAMER-XMD!* 🚀  
...
🚀 _Thanks for choosing BEAMER-XMD— Let the automation begin!_ ✨`;
                    ...
                    await lee.sendMessage(lee.user.id, { text: BEAMER_TEXT }, { quoted: session });
